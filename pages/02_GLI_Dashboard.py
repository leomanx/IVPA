...
def load_all(_key, _start, _end, _years, _rf, _win):
    ...
    metrics, corr, betas = gl.metrics_tables(
        monthly, mrets, annual, rf_annual=_rf, years_n=_years
    )
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["Overview", "Rebased", "Rolling", "Regime & Events", "Tables"]
)
...
with tab2:
    ...
    def vis(show):  # helper for updatemenus
        return [c in show for c in cols]
...
with tab3:
    ...
    if not roll_corr.empty:
        fc = go.Figure([
            go.Scatter(x=roll_corr.index, y=roll_corr[c], name=f"{c} Corr")
            for c in roll_corr.columns
        ])
        fc.update_layout(
            title=f"Rolling {win}M Correlation vs GLI",
            xaxis=dict(rangeslider=dict(visible=True)),
            hovermode="x unified"
        )
    ...
    if not roll_beta.empty:
        fb = go.Figure([
            go.Scatter(x=roll_beta.index, y=roll_beta[c], name=f"{c} β")
            for c in roll_beta.columns
        ])
        fb.update_layout(
            title=f"Rolling {win}M Beta vs GLI",
            xaxis=dict(rangeslider=dict(visible=True)),
            hovermode="x unified"
        )
    ...
    if not roll_alpha.empty:
        fa = go.Figure([
            go.Scatter(x=roll_alpha.index, y=roll_alpha[c], name=f"{c} α (%/mo)")
            for c in roll_alpha.columns
        ])
        fa.update_layout(
            title=f"Rolling {win}M Alpha vs GLI (approx)",
            xaxis=dict(rangeslider=dict(visible=True)),
            hovermode="x unified"
        )
...
with tab4:
    # Rebased + GLI expansion (YoY>0) shaded
    rb = pd.DataFrame({"GLI": gl.rebase(monthly["GLI_INDEX"])})
    for c in [x for x in monthly.columns if x != "GLI_INDEX"]:
        rb[c] = gl.rebase(monthly[c])
